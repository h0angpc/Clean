...
const QuickPopupHelper: React.FC<QuickPopupHelper> = ({ toggle }) => {
  //assume that you have booking state
  const bookingState: string = "Confirmed"
  const style = 
  ...
  bookingState === "Confirmed" 
    ?(<div className='w-[60%] h-full bg-[#6225ee] p-[13px] bg-opacity-20 rounded-lg'>
    <p className='flex h-full justify-center items-center text-[#6225ee] text-xl font-bold'>{bookingState}</p>
    </div>): "";

  const styleBtn = 
  bookingState === "Completed" 
    ?(<Button className="w-full h-[55px] bg-[#1A78F2] text-lg text-white font-Averta-Semibold">Having an Issue</Button>): 
  bookingState === "In Progress" 
    ?(<Button className="w-full h-[55px] bg-[#1A78F2] text-lg text-white font-Averta-Semibold">End Working Session</Button>):
  bookingState === "Confirmed" 
    ?(<Button className="w-full h-[55px] bg-[#1A78F2] text-lg text-white font-Averta-Semibold">Start Working Session</Button>): "";

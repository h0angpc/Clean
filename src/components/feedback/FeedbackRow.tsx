...
const FeedbackRow: React.FC<FeedbackRowProps> = ({
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (onCheckboxToggle) {
      onCheckboxToggle(feedback.id, checked);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex flex-[3] flex-col px-3 py-4 xl:justify-center font-bold text-center whitespace-nowrap `}
      >
        <div className="flex overflow-hidden flex-1 items-center xl:justify-center size-full">
          <div className="flex flex-row items-center gap-2 self-stretch my-auto w-[93px]">
            ...
            <div
              className={`flex relative gap-4 justify-between text-xs items-start px-4 py-1.5 min-h-[27px] min-w-24 ${sentimentColor} rounded-md`}
            >
